...
export const MemberSidebar: React.FC = () => {
  const { servers, currentServerId, isMembersListOpen, setMembersListOpen, selectChannel, selectServer } = useDiscord();
  const [selectedMember, setSelectedMember] = useState<Member | null>(null);

  if (currentServerId === 'home') return null;
  ...
  const handleSendDm = (member: Member) => {
    ...
    setMembersListOpen(false); // Close drawer on mobile
  };

  return (
    <>
      {/* Mobile Backdrop for Members Drawer */}
      {isMembersListOpen && (
        <div 
          onClick={() => setMembersListOpen(false)}
          className="fixed inset-0 bg-black/50 backdrop-blur-xs z-30 lg:hidden transition-all duration-300 animate-fadeIn"
        />
      )}

      {/* Responsive Member Sidebar (Slides out on mobile, static on desktop) */}
      <div className={`fixed lg:static top-0 bottom-0 right-0 z-40 w-60 bg-discord-dark-300 dark:bg-discord-dark-300 border-l border-discord-dark-500 flex flex-col p-4 select-none h-full shrink-0 transition-transform duration-300 lg:translate-x-0 ${
        isMembersListOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'
      }`}>
        
        {/* Mobile Header Close Button */}
        <div className="flex items-center justify-between lg:hidden border-b border-discord-dark-500 pb-2 mb-4 text-white">
          <span className="font-bold text-sm">Server Members</span>
          <button 
            onClick={() => setMembersListOpen(false)}
            className="p-1 rounded hover:bg-discord-dark-100 text-discord-light-300 hover:text-white"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {selectedMember && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn" onClick={() => setSelectedMember(null)}>
            <div 
              className="bg-discord-dark-200 rounded-lg w-full max-w-[340px] text-white overflow-hidden shadow-2xl border border-discord-dark-100/10 animate-scaleIn"
