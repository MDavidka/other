// Auto-generated. Snapshot of the design brief, theme tokens and
// page plan that produced this site. Useful for follow-up generation
// passes and for diffing edits between regenerations.
export const generatedManifest = {
  "brief": {
    "projectName": "other",
    "tagline": "Premium phones, zero compromise.",
    "description": "other is the modern destination for flagship smartphones, expert curation, and fast, reliable delivery. We stock the latest devices from Apple, Samsung, Google, and Sony with transparent pricing and hands-on support.",
    "audience": "Tech-forward consumers seeking premium devices and seamless buying experience",
    "voice": "Confident, crisp, and helpful",
    "themePreset": "ecommerce",
    "navLinks": [
      {
        "label": "Home",
        "href": "/"
      },
      {
        "label": "Shop",
        "href": "/shop"
      },
      {
        "label": "Compare",
        "href": "/compare"
      },
      {
        "label": "Support",
        "href": "/support"
      }
    ],
    "primaryCta": {
      "label": "Browse phones",
      "href": "/shop"
    },
    "secondaryCta": {
      "label": "Compare models",
      "href": "/compare"
    },
    "footerCta": {
      "label": "Talk to an expert",
      "href": "/support"
    },
    "contact": {
      "email": "hello@other.store",
      "phone": "+1 (415) 555-0192",
      "address": "142 Market St, San Francisco, CA"
    },
    "logoUrl": "data:image/svg+xml;base64,",
    "logoInitials": "O",
    "socialLinks": [
      {
        "label": "Instagram",
        "href": "https://instagram.com/other"
      },
      {
        "label": "Twitter",
        "href": "https://twitter.com/other"
      }
    ]
  },
  "theme": {
    "preset": "ecommerce",
    "light": {
      "background": "0 0% 100%",
      "foreground": "240 10% 8%",
      "card": "0 0% 100%",
      "cardForeground": "240 10% 8%",
      "popover": "0 0% 100%",
      "popoverForeground": "240 10% 8%",
      "primary": "346 78% 50%",
      "primaryForeground": "0 0% 100%",
      "secondary": "240 5% 96%",
      "secondaryForeground": "240 6% 14%",
      "muted": "240 5% 96%",
      "mutedForeground": "240 4% 46%",
      "accent": "30 100% 96%",
      "accentForeground": "346 78% 35%",
      "destructive": "0 84% 60%",
      "destructiveForeground": "0 0% 98%",
      "border": "240 6% 90%",
      "input": "240 6% 90%",
      "ring": "346 78% 50%"
    },
    "dark": {
      "background": "240 10% 6%",
      "foreground": "0 0% 98%",
      "card": "240 8% 9%",
      "cardForeground": "0 0% 98%",
      "popover": "240 8% 9%",
      "popoverForeground": "0 0% 98%",
      "primary": "346 85% 60%",
      "primaryForeground": "0 0% 100%",
      "secondary": "240 5% 14%",
      "secondaryForeground": "0 0% 98%",
      "muted": "240 5% 14%",
      "mutedForeground": "240 5% 65%",
      "accent": "346 50% 20%",
      "accentForeground": "346 85% 92%",
      "destructive": "0 72% 51%",
      "destructiveForeground": "0 0% 98%",
      "border": "240 5% 18%",
      "input": "240 5% 18%",
      "ring": "346 85% 60%"
    },
    "radius": "1rem",
    "fontSans": "\"Inter\", system-ui, sans-serif",
    "fontDisplay": "\"Inter\", system-ui, sans-serif",
    "background": "soft"
  },
  "designDirection": {
    "concept": "Clean, modern retail with editorial product focus and subtle motion that highlights device craftsmanship",
    "visualStyle": "commerce-catalog",
    "layoutRhythm": "conversion-focused",
    "density": "balanced",
    "motionLevel": "subtle",
    "trustStrategy": "stats",
    "imageStrategy": "product",
    "avoid": [
      "dense card grids",
      "loud gradients",
      "overly decorative elements"
    ]
  },
  "pages": [
    {
      "path": "/",
      "title": "other — Premium phones",
      "metaTitle": "other — Premium Flagship Phones & Fast Delivery",
      "metaDescription": "Discover curated flagship smartphones from Apple, Samsung, Google, and Sony. Transparent pricing, expert support, and 2-day shipping.",
      "sections": [
        {
          "kind": "hero",
          "variant": "cinematic",
          "description": "Shop the latest iPhone, Galaxy, Pixel, and Xperia models with transparent pricing and expert guidance.",
          "secondaryCta": {
            "label": "Compare models",
            "href": "/compare"
          }
        },
        {
          "kind": "stats",
          "variant": "row",
          "items": [
            {
              "label": "Phones shipped last month",
              "value": "42,800",
              "suffix": "+",
              "highlighted": false
            },
            {
              "label": "Average customer rating",
              "value": "4.98",
              "highlighted": false
            },
            {
              "label": "Day delivery guarantee",
              "value": "2",
              "suffix": "days",
              "highlighted": false
            }
          ]
        },
        {
          "kind": "feature-grid",
          "variant": "asymmetric-bento",
          "description": "Every device is inspected, photographed, and backed by our 30-day return promise.",
          "items": [
            {
              "title": "Verified battery health",
              "description": "Every phone ships with 85%+ battery capacity and full diagnostic report.",
              "highlighted": false
            },
            {
              "title": "Price-match guarantee",
              "description": "Find a lower price within 7 days and we refund the difference plus 10%.",
              "highlighted": false
            },
            {
              "title": "Same-day expert support",
              "description": "Real humans answer within 4 minutes. No chatbots, no scripted replies.",
              "highlighted": false
            },
            {
              "title": "Unlocked & carrier-ready",
              "description": "All devices work on every major US carrier with free SIM kit included.",
              "highlighted": false
            }
          ]
        },
        {
          "kind": "testimonials",
          "variant": "grid-cards",
          "items": [
            {
              "quote": "Switched from my 12 Pro to the 16 Pro Max. Delivery arrived next morning and setup took 11 minutes.",
              "author": "Marcus Rivera",
              "role": "Product Designer, Stripe",
              "initials": "MR",
              "highlighted": false
            },
            {
              "quote": "Finally found a store that shows real trade-in values. Got $420 for my old S22 and the new S25 shipped the same day.",
              "author": "Priya Patel",
              "role": "Founder, Arc Studio",
              "initials": "PP",
              "highlighted": false
            },
            {
              "quote": "Pixel 9 Pro arrived with 92% battery health and a handwritten note. Support answered in under 3 minutes.",
              "author": "David Chen",
              "role": "Engineering Manager, Vercel",
              "initials": "DC",
              "highlighted": false
            }
          ]
        },
        {
          "kind": "pricing",
          "variant": "two-tier-toggle",
          "description": "Flexible financing and protection options that grow with your device.",
          "items": [
            {
              "title": "Pay in full",
              "price": "0",
              "period": "one-time",
              "features": [
                "No interest charges",
                "Instant ownership",
                "Full 30-day returns",
                "Lifetime support access"
              ],
              "highlighted": false
            },
            {
              "title": "Finance 0% APR",
              "price": "24",
              "period": "months",
              "features": [
                "0% interest for 24 months",
                "Flexible monthly payments",
                "Early payoff allowed",
                "Device protection included"
              ],
              "highlighted": true
            }
          ]
        },
        {
          "kind": "faq",
          "variant": "accordion",
          "items": [
            {
              "title": "Do phones come with a warranty?",
              "description": "All devices include a 12-month limited warranty plus 30-day return window. Extended coverage available at checkout.",
              "highlighted": false
            },
            {
              "title": "Can I trade in my current phone?",
              "description": "Yes. Get an instant quote online and receive credit within 48 hours after we inspect the device.",
              "highlighted": false
            },
            {
              "title": "Which carriers work with these phones?",
              "description": "Every phone is carrier-unlocked and compatible with AT&T, Verizon, T-Mobile, and US Cellular.",
              "highlighted": false
            },
            {
              "title": "How fast is delivery really?",
              "description": "Orders placed before 2pm ship same day. 95% of customers receive devices within 48 hours.",
              "highlighted": false
            }
          ]
        },
        {
          "kind": "cta",
          "variant": "boxed-card",
          "description": "Browse our full catalog or speak with a specialist about the right device for you.",
          "secondaryCta": {
            "label": "Talk to sales",
            "href": "/support"
          }
        }
      ]
    },
    {
      "path": "/shop",
      "title": "Shop phones",
      "metaTitle": "Shop Flagship Phones — other",
      "metaDescription": "Browse the latest iPhone, Galaxy, Pixel, and Xperia models. Filter by brand, storage, and price. 2-day shipping included.",
      "sections": [
        {
          "kind": "hero",
          "variant": "editorial",
          "description": "Filter by brand, storage, camera specs, and price. Every device ships same day with full diagnostic report."
        },
        {
          "kind": "product-grid",
          "variant": "card-grid",
          "description": "Hand-selected devices with verified battery health and carrier unlock status.",
          "items": [
            {
              "title": "iPhone 16 Pro Max",
              "description": "256GB • Titanium • 48MP camera",
              "price": "1199",
              "highlighted": false,
              "category": "Apple",
              "tag": "New"
            },
            {
              "title": "Galaxy S25 Ultra",
              "description": "512GB • Titanium • 200MP camera",
              "price": "1299",
              "highlighted": false,
              "category": "Samsung",
              "tag": "New"
            },
            {
              "title": "Pixel 9 Pro XL",
              "description": "256GB • Obsidian • 50MP camera",
              "price": "999",
              "highlighted": false,
              "category": "Google",
              "tag": "Best value"
            },
            {
              "title": "Xperia 1 VI",
              "description": "256GB • Black • 48MP camera",
              "price": "1399",
              "highlighted": false,
              "category": "Sony",
              "tag": "Pro"
            }
          ]
        }
      ]
    },
    {
      "path": "/compare",
      "title": "Compare phones",
      "metaTitle": "Compare Flagship Phones — other",
      "metaDescription": "Side-by-side comparison of iPhone 16 Pro Max, Galaxy S25 Ultra, Pixel 9 Pro XL, and Xperia 1 VI across specs and pricing.",
      "sections": [
        {
          "kind": "hero",
          "variant": "editorial",
          "description": "Compare camera, battery, display, and performance metrics across the four leading flagships."
        },
        {
          "kind": "comparison",
          "variant": "table",
          "items": [
            {
              "title": "Display",
              "description": "6.9\" 120Hz OLED",
              "highlighted": false
            },
            {
              "title": "Camera",
              "description": "48MP main + 48MP ultra",
              "highlighted": false
            },
            {
              "title": "Battery",
              "description": "4685 mAh • 45W charging",
              "highlighted": false
            },
            {
              "title": "Storage options",
              "description": "256GB / 512GB / 1TB",
              "highlighted": false
            },
            {
              "title": "Starting price",
              "description": "$1199",
              "highlighted": false
            }
          ]
        }
      ]
    },
    {
      "path": "/support",
      "title": "Support",
      "metaTitle": "Expert Support — other",
      "metaDescription": "Talk to real humans who know phones. Get setup help, trade-in quotes, or answers to technical questions.",
      "sections": [
        {
          "kind": "hero",
          "variant": "centered",
          "description": "Our phone specialists answer in under 4 minutes. No scripts, no chatbots."
        },
        {
          "kind": "contact",
          "variant": "split-form",
          "description": "Tell us your device and question. A real expert replies within minutes."
        }
      ]
    }
  ],
  "deploymentMode": "next-server",
  "needsDatabase": true,
  "databaseProvider": "turso",
  "integrations": [
    {
      "kind": "database",
      "name": "Turso",
      "provider": "turso",
      "reason": "Store product inventory, order history, and customer preferences with edge SQL",
      "envVars": [
        "TURSO_DATABASE_URL",
        "TURSO_AUTH_TOKEN"
      ]
    }
  ],
  "requiredEnvVars": [
    {
      "key": "TURSO_DATABASE_URL",
      "purpose": "Turso — Store product inventory, order history, and customer preferences with edge SQL",
      "provider": "turso",
      "required": true,
      "integration": "Turso"
    },
    {
      "key": "TURSO_AUTH_TOKEN",
      "purpose": "Turso — Store product inventory, order history, and customer preferences with edge SQL",
      "provider": "turso",
      "required": true,
      "integration": "Turso"
    }
  ],
  "unconnectedIntegrations": []
} as const

export type GeneratedManifest = typeof generatedManifest
